...
static void *MremapCreateAlias(uptr base_addr, uptr alias_addr,
                               uptr alias_size) {
  return mremap(reinterpret_cast<void *>(base_addr), 0, alias_size,
...
static void CreateAliases(uptr start_addr, uptr alias_size, uptr num_aliases) {
  ...
  for (uptr i = 1; i < num_aliases; ++i) {
    ...
    void *alias = MremapCreateAlias(start_addr, alias_addr, alias_size);
    CHECK_EQ(reinterpret_cast<uptr>(alias), alias_addr);
  }
}
